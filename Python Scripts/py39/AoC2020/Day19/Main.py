...
def parse(day:int,test:int, *arg, **args):
    """ imports input file from r'AoC2020\Day{day}\{f}'  """ # pylint: disable=anomalous-backslash-in-string
...
def rules_builder(rules, rule_dict):
    ...
    for each in rules:
        ...
        if not '|' in value:
            ...
            value=value.split(' | ')
            l1 = lambda v=value[0].split(' '), r = rule_dict : ''.join([r[each]() if each.isdigit() else each for each in v]) # pylint: disable=unsubscriptable-object
            l2 = lambda v=value[1].split(' '), r = rule_dict : ''.join([r[each]() if each.isdigit() else each for each in v]) # pylint: disable=unsubscriptable-object
