...
class cart():
    ...
    def remove_carts(self,broken_carts = ()):
        for each in broken_carts:
            print (each.position, self.track.parent.carts)
            self.track.parent.carts[each.position] = (-1,-1)
...
class plan():
    ...
    def update(self):
        self.ticks += 1
        # find all locations that have a cart
        order = self.carts.keys()
        # sort these carts from upperleft to right
        order.sort()
        new_dict = dict()
        # for each cart on the map
        for i in range(len(order)):
            # get the new position of that cart
            # move updates the track and the cart qua position
            coord = self.carts[order[i]].move()
            # if a track contains more than 1 cart
            if len(self.carts[order[i]].track.carts) > 1:
                # for each of those carts
                for each in self.carts[order[i]].track.carts:
                    # if it was not already noted as crashed
                    if each not in self.crashes:
                        # note that it crashed
                        self.crashes.append(each)
            new_dict[coord] = self.carts[order[i]]
        removed = []

        for each in self.crashes:
            if each.position in new_dict and each == new_dict[each.position]:
                each.track.carts = []
                each.track = None
                new_dict.pop(each.position)
                removed.append(each)
        self.carts = new_dict
